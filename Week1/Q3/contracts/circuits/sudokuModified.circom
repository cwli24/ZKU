...
template sudoku() {
    ...
    component puzRng[9][9];
    component solRng[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            //[assignment] change assert() to use your created RangeProof instead
            puzRng[i][j] = RangeProof(4);
            puzRng[i][j].range[0] <== 0;
            puzRng[i][j].range[1] <== 9;
            puzRng[i][j].in <== puzzle[i][j];
            puzRng[i][j].out === 1;

            //[assignment] change assert() to use your created RangeProof instead
            solRng[i][j] = RangeProof(4);
            solRng[i][j].range[0] <== 0;
            solRng[i][j].range[1] <== 9;
            solRng[i][j].in <== solution[i][j];
            solRng[i][j].out === 1;
